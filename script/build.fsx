...
let debugNuspecPath = Path.GetFullPath(@"./packaging/nuget/couchbase-lite-apx-debug.nuspec")
...
Target "Package" (fun _ ->
    ...
    let debugNuspecProps = getNuspecProperties(File.ReadAllText(debugNuspecPath))
    ...
    let debugCommandArgs = sprintf @"pack -Verbosity detailed -BasePath %s -Version %s -OutputDirectory %s %s" basePath debugVersion artifactsNuGetDir debugNuspecPath
    let debugResult = Shell.Exec(nugetPath,  debugCommandArgs.ToString())
